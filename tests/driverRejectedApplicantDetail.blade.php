...
<h1 style="text-align:center;"> {!! $title or 'New Applicant Details' !!} </h1>
    ...
    <div class="col-sm-4">
	 ...
	 <!--Subadmin Permission Code Start-->
		<?php 
			...
			?>
				<!--Subadmin Permission Code Start-->	
				  <?php if(!empty($driver_meta_data->ssn)){@$decrypted_string = \Illuminate\Support\Facades\Crypt::decrypt(@$driver_meta_data->ssn); }?>
				  <p><span>SSN/TIN:</span>{{ @$decrypted_string }} </p>
			<?php 
			}else if(!empty($loggedInUserPermission)){
			?>		
				 <?php if(!empty($driver_meta_data->ssn)){
							@$decrypted_string = \Illuminate\Support\Facades\Crypt::decrypt(@$driver_meta_data->ssn);
							$ssn = '#####'.substr($decrypted_string, -4);
					  }
				  ?>
